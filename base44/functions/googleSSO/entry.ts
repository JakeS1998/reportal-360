...
export default async function(req) {
  try {
    ...
    if (teachers.length > 0) {
      ...
      await logAudit(base44, "user_created", "SSO", "admin", `Google SSO auto-provisioned user ${finalUsername} (${email})`);
      await logAudit(base44, "login_success", finalUsername, "teacher", "SSO login successful (Google, new user)", undefined, { ip_address: ip, user_agent: userAgent });
    }
