...
import {
  KeyRound, Plus, RefreshCw, Trash2, Power, Copy, Check, Building2,
} from "lucide-react";
...
export default function AccessCodeManager() {
  ...
  const [form, setForm] = useState({ systemCode: "", schoolCode: "", schoolName: "", code: randomCode(), expires_at: "", notes: "" });
  ...
  const pickSystem = async (systemCode) => {
    ...
    setForm((f) => ({ ...f, systemCode, schoolCode: "", schoolName: "" }));
    setSchools([]);
  ...
  const create = async (e) => {
    e.preventDefault();
    if (!form.systemCode || !form.schoolCode || !form.code) return;
    setSaving(true);
    setError("");
    try {
      const res = await base44.functions.invoke("manageAccessCodes", {
        action: "create",
      ...
      setForm({ systemCode: "", schoolCode: "", schoolName: "", code: randomCode(), expires_at: "", notes: "" });
      setSchools([]);
      loadCodes();
    } catch (err) {
  ...
  return (
    <SectionCard title="Access Codes" subtitle="Generate and manage subscriber access codes" icon={KeyRound}>
      <form onSubmit={create} className="bg-slate-50 rounded-xl p-4 space-y-4 border border-slate-100">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Create new code</p>
        ...
        <Button type="submit" disabled={saving} className="bg-slate-900 hover:bg-slate-800">
          <Plus className="w-4 h-4 mr-1" /> {saving ? "Creating…" : "Create Access Code"}
        </Button>
      </form>
                ...
                <div key={c.id} className="flex items-center gap-3 bg-white border border-slate-200 rounded-lg px-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500 truncate">
                      {c.school_name || `School ${c.school_code}`}
                      {c.notes ? ` · ${c.notes}` : ""}
