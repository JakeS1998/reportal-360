...
export default function BulkStaffUpload({ callerCreds, school, onImported }) {
  ...
  return <SectionCard title="Bulk Add Staff" subtitle="Upload a CSV to create multiple teacher or school manager accounts" icon={Upload}>
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2"><input type="file" accept=".csv,text/csv" onChange={(event) => setFile(event.target.files?.[0] || null)} className="text-sm" /><Button onClick={upload} disabled={!file || loading} size="sm">{loading ? "Uploading…" : "Upload staff file"}</Button></div>
      {result && <div className="text-sm"><p className={result.success ? "text-emerald-700" : "text-rose-600"}>{result.success ? `${result.count} account${result.count === 1 ? "" : "s"} created.` : result.error}</p>{result.errors?.length > 0 && <p className="text-rose-600 mt-1">Skipped: {result.errors.join(" · ")}</p>}{result.credentials?.length > 0 && <Button onClick={downloadCredentials} variant="outline" size="sm" className="mt-2"><Download className="w-3.5 h-3.5 mr-1" /> Download credentials</Button>}</div>}
    </div>
  </SectionCard>;
}