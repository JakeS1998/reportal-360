...
export default function BulkStaffUpload({ callerCreds, school, onImported }) {
  ...
  const downloadTemplate = () => {
    const blob = new Blob(["full_name,email,role,username,password,subject,grades,room\n"], { type: "text/csv;charset=utf-8" });
  ...
  return <SectionCard title="Bulk Add Staff" subtitle="Upload a CSV to create multiple teacher or school manager accounts" icon={Upload}>
    <div className="space-y-3">
      <p className="text-xs text-slate-500">Use columns: <span className="font-mono">full_name,email,role,username,password,subject,grades,room</span>. Separate multiple subjects and grades with a semicolon (for example, <span className="font-mono">Math;Science</span> and <span className="font-mono">6;7;8</span>). Role is optional and defaults to teacher.</p>
