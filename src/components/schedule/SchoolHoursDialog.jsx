...
import { Coffee, UtensilsCrossed, AlertTriangle } from "lucide-react";

export default function SchoolHoursDialog({ open, onOpenChange, schoolCode, onSaved }) {
  const [timetableId, setTimetableId] = useState(null);
  const [form, setForm] = useState({ break_start: "", break_end: "", lunch_start: "", lunch_end: "" });
    ...
    base44.entities.SchoolTimetable.filter({ school_code: schoolCode })
      .then((rows) => {
        if (rows.length > 0) {
          ...
          setTimetableId(null);
          setForm({ break_start: "09:30", break_end: "09:45", lunch_start: "12:00", lunch_end: "12:45" });
        }
      })
      .catch(() => {});
  }, [open, schoolCode]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>School Break &amp; Lunch Times</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <p className="text-xs text-slate-400">These times are shaded across the weekly grid and excluded from each teacher's free-period count.</p>
