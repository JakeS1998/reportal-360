...
import SchoolAccessAuthorizationDialog from "@/components/SchoolAccessAuthorizationDialog";
import { Building2 } from "lucide-react";

export default function SchoolAccessManager({ callerCreds, adminUser }) {
  const [schools, setSchools] = useState([]); const [query, setQuery] = useState(""); const [schoolKey, setSchoolKey] = useState(""); const [reason, setReason] = useState(""); const [error, setError] = useState(""); const [saving, setSaving] = useState(false); const [progressStage, setProgressStage] = useState(""); const [authorizationOpen, setAuthorizationOpen] = useState(false);
