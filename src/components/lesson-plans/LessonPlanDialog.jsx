...
import LessonPlanPrintButton from "@/components/lesson-plans/LessonPlanPrintButton";
...
export default function LessonPlanDialog({ open, onOpenChange, context, onSaved }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Complete Lesson Plan</DialogTitle></DialogHeader><div className="space-y-3"><div><Label>Lesson title</Label><Input className="mt-1" value={plan.title} disabled={approved} onChange={(e) => update("title", e.target.value)} /></div>{[["objectives", "Learning objectives"], ["activities", "Activities"], ["resources", "Resources and materials"], ["assessment", "Assessment and evidence"]].map(([key, label]) => <div key={key}><Label>{label}</Label><Textarea className="mt-1 min-h-20" value={plan[key]} disabled={approved} onChange={(e) => update(key, e.target.value)} /></div>)}<label className="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={plan.shared} disabled={approved} onChange={(e) => update("shared", e.target.checked)} /> Share this plan with other teachers</label></div><DialogFooter><LessonPlanPrintButton plan={plan} /><Button variant="outline" onClick={() => onOpenChange(false)}>{approved ? "Close" : "Cancel"}</Button>{!approved && <><Button variant="outline" disabled={saving || !plan.title.trim()} onClick={() => save("draft")}>Save Draft</Button><Button disabled={saving || !plan.title.trim()} onClick={() => save("pending_review")}>{saving ? "Saving…" : "Submit for Review"}</Button></>}</DialogFooter></DialogContent></Dialog>;
}