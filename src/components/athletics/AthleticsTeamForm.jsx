...
export default function AthleticsTeamForm({ schoolCode, user, onCreated }) {
  ...
  const submit = async (e) => { e.preventDefault(); setSaving(true); const team = await base44.entities.AthleticsTeam.create({ ...form, school_code: schoolCode, coach_id: user?.teacher_id || user?.id || "", coach_name: user?.full_name || user?.username || "Coach" }); setForm({ name: "", sport: "", season: "", level: "Varsity" }); setSaving(false); onCreated(team); };
