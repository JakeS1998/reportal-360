...
export default function SeatingPlan() {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-900">Classroom designer</h2><p className="text-sm text-slate-500">{classInfo.class_name} · drag desks anywhere, then choose a desk to assign a student.</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setGridlines((visible) => !visible)}><Grid3X3 className="mr-1 h-4 w-4" /> {gridlines ? "Hide grid" : "Show grid"}</Button><Button size="sm" variant="outline" onClick={resetUShape}><RotateCcw className="mr-1 h-4 w-4" /> U-shape</Button><Button size="sm" onClick={savePlan} disabled={saving}><Save className="mr-1 h-4 w-4" /> {saving ? "Saving…" : "Save plan"}</Button></div></div>
