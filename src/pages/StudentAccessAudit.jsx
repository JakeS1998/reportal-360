...
export default function StudentAccessAudit() {
  ...
  return <SectionCard title="Student Access Audit" subtitle="Teacher access to student records" icon={ShieldCheck}>{loading ? <div className="h-48 animate-pulse rounded-xl bg-slate-100" /> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs text-slate-400"><th className="pb-3">Time</th><th className="pb-3">Teacher</th><th className="pb-3">Role</th><th className="pb-3">Student</th></tr></thead><tbody>{logs.length ? logs.map((log) => <tr key={log.id} className="border-b border-slate-100"><td className="py-3 text-slate-500">{new Date(log.created_date).toLocaleString()}</td><td className="py-3 font-medium text-slate-800">{log.username || "—"}</td><td className="py-3 text-slate-500">{log.user_role || "—"}</td><td className="py-3 font-medium text-slate-600">{log.student_name || "Student record unavailable"}</td></tr>) : <tr><td colSpan="4" className="py-10 text-center text-slate-400">No student access has been recorded.</td></tr>}</tbody></table></div>}</SectionCard>;
}